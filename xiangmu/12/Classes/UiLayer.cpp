...
bool UiLayer::init(){
	...
	this->waves=0;
	//UI½çÃæ
	uilayer = GUIReader::getInstance()->widgetFromJsonFile("NewUi_1.ExportJson");
...
//		    	auto label = Label::create("Liu tian hou you tiao pi le","Marker Felt.ttf",100);
//			label->setPosition(size/2);
//			label->setColor(Color3B::YELLOW);
//			label->setName("liutianhou");
//			this->addChild(label);
                        auto image = uilayer->getChildByName("Image_3");
			image->setVisible(!image->isVisible());
			SimpleAudioEngine::getInstance()->playEffect("homediaoxue.mp3");
		}
...
//			label->runAction(Sequence::create(DelayTime::create(0.1),remove,NULL));
	...
	mainmenu->addTouchEventListener([=](Ref* sender,Widget::TouchEventType type){
		if(type==Widget::TouchEventType::BEGAN){
			SimpleAudioEngine::getInstance()->playEffect("ShootSelect.mp3");
		}
		if(type==Widget::TouchEventType::ENDED){
	...
	BaseTower* tower = (BaseTower*)sender;
 	if(tower->getCurgold()<curmoney){
 		curmoney-=tower->getCurgold();
                SimpleAudioEngine::getInstance()->playEffect("TowerUpdata.mp3");
 		tower->uplevel();
 	}else{
		auto size = Director::getInstance()->getWinSize();
		...
		auto nomoneylabel = Label::create("Lack of money","Marker Felt.ttf",30);
