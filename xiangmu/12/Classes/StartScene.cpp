...
bool StartScene::init(){
	...
	auto uilayer = GUIReader::getInstance()->widgetFromJsonFile("Start.json");
	...
	btnStart->addTouchEventListener([](Ref* ref,Widget::TouchEventType t){
		if (t == Widget::TouchEventType::ENDED)
		{
			...
			SimpleAudioEngine::getInstance()->playEffect("btnOver.mp3");
		}
	...
	auto isMusicON = UserDefault::getInstance()->getBoolForKey("isMusicON",true);
	 instance->playBackgroundMusic("battle1.mp3",true);
	if (isMusicON)
	{     
