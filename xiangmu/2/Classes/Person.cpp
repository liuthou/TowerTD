...
void Person::readXml(std::string strXml)
{
	...
	std::string wayXml = FileUtils::getInstance()->getStringFromFile(strXml);
	tinyxml2::XMLDocument doc;
	doc.Parse(wayXml.c_str());
...
void Person::die(){
	heroDie = true;
	RoleAmrature::getInstance()->setRoleMoney(RoleAmrature::getInstance()->getRoleMoney() + 100);
	RoleAmrature::getInstance()->setRoleExp(RoleAmrature::getInstance()->getRoleExp() + 100);
...
void Person::skill(){
	...
	this->stopAllActions();
	auto skillAnimation = CallFunc::create([=](){
		this->getAnimation()->play(animationSkill);
	});
	auto time = DelayTime::create(1);
	auto moveAnimation = CallFunc::create([=](){
		this->setArmaturestatic(moveStatic);
	});
	auto seq = Sequence::create(skillAnimation,time,moveAnimation,nullptr);
	this->runAction(seq);
}
...
ARMATURESTATIC Person::getArmaturestatic(){
	return this->armaturestatic;
}