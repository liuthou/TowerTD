...
bool GameOverLayer::init(int type){
	...
	if (type == 0)
	{
	    uilayer = GUIReader::getInstance()->widgetFromJsonFile("startUI/GameWin.json");
	...
	if (type == 1)
	{
		uilayer = GUIReader::getInstance()->widgetFromJsonFile("startUI/GameDefat.json");
		this->addChild(uilayer);
	}
	if (type == 2)
	{
		uilayer = GUIReader::getInstance()->widgetFromJsonFile("startUI/GameOver.json");
		this->addChild(uilayer);
	}
