...
void UpdateLayer::show()
{
    ...
    //点击会进入到升级
	 ...
	 //升级标签
	 updateLabel= Label::createWithTTF("100","fonts/Marker Felt.ttf",30);
	...
	MenuItemImage * item2 = MenuItemImage::create("delete.png","asd.png",[=](Ref *){
		...
		SimpleAudioEngine::getInstance()->playEffect("musics/arrowTower.wav");
		towers->postDeleteTower();
		auto dt = CallFunc::create(CC_CALLBACK_0(UpdateLayer::unshow,this));
		this->runAction( Sequence::createWithTwoActions(DelayTime::create(0.1f),dt));
	});
	//铲除标签
	auto eradicantLabel= Label::createWithTTF("200","fonts/Marker Felt.ttf",30);
	...
	//种塔范围背景
	 auto towerBj =  Sprite::create("slice01_01.png");
	 towerBj->setName("towerBj");
	 towerBj->setPosition(Vec2(towers->getPosition()));
	 towerBj->setScale(towers->getScope()/towerBj->getContentSize().width);
	...
	//铲除你获得的钱
     ...
     if(towers->getCurlevel()>3)
	 {
       item1->setColor(Color3B::GRAY);
		 auto UpleveLabel= Label::createWithTTF("Max","fonts/Marker Felt.ttf",30);
